...
class AssetManager(metaclass=SingletonMeta):
    async def change_asset_pull_mission_status(
        self, asset_owner_type: str, asset_owner_id: int, active: bool
    ):
        mission_obj = (
            await EveAssetPullMissionDBUtils.select_mission_by_owner_id_and_owner_type(
                asset_owner_id, asset_owner_type
            )
        )
        if not mission_obj:
    ...
    async def pull_asset_now(self, asset_owner_type: str, asset_owner_id: int):
        ...
        await rdm().r.hset(
            f"asset_pull_mission_status:{asset_owner_type}:{asset_owner_id}",
            "step_name",
            "清理旧数据",
        )
    ...
    async def get_user_asset_pull_mission_list(self, user_name: str) -> list[dict]:
        missions = []
        mission_keys = set()  # (subject_type, subject_id) 用于去重

        def _append_mission(m: dict):
        ...
        # 拉取个人创建的任务
        async for mission in await EveAssetPullMissionDBUtils.select_all_by_user_name(
            user_name
        ):
            try:
                if mission.asset_owner_type == "character":
                    character = await CharacterManager().get_character_by_character_id(
                        mission.asset_owner_id
                    )
                    subject_name = character.character_name
                elif mission.asset_owner_type == "corp":
                    corporation = (
                        await CharacterManager().get_corporation_data_by_corporation_id(
                            mission.asset_owner_id
                        )
                    )
                ...
                _append_mission(
                    {
                        ...
                        "last_pull_time": mission.last_pull_time.replace(
                            tzinfo=timezone(timedelta(hours=+8), "Shanghai")
                        )
                        if mission.last_pull_time
                        else None,
                        "is_abnormal": False,
                    }
                )
                ...
                traceback.print_exc()
                logger.error(
                    f"拉取个人创建的任务失败: {e}, character_id: {mission.asset_owner_id}"
                )
                _append_mission(
                    {
                        "subject_type": mission.asset_owner_type,
        ...
        main_character = (
            await CharacterManager().get_character_by_character_id(main_character_id)
            if main_character_id
            else None
        )
        if main_character:
            logger.info(
                f"主角色: {main_character.character_name} {main_character.corporation_id}"
            )
        if main_character and main_character.corporation_id:
            corp_id = main_character.corporation_id
            async for (
                mission
            ) in await EveAssetPullMissionDBUtils.select_all_by_owner_id_and_owner_type(
                corp_id, "corp"
            ):
                try:
                    if (
                        mission.asset_owner_type,
                        mission.asset_owner_id,
                    ) not in mission_keys:
                        logger.info(f"拉取同公司的任务: {mission.asset_owner_id}")
                        corporation_info = await rdm().redis.get(
                            f"eveesi:corporations_corporation:{mission.asset_owner_id}"
                        )
                        if not corporation_info:
                            corporation_info = await eveesi.corporations_corporation_id(
                                mission.asset_owner_id
                            )
                            logger.info(f"esi res:{corporation_info}")
                            if not corporation_info:
                                logger.error(
                                    f"公司{mission.asset_owner_id}获取公开信息失败，标记为异常"
                                )
                                _append_mission(
                                    {
                                        "subject_type": mission.asset_owner_type,
                            ...
                            await rdm().redis.set(
                                f"eveesi:corporations_corporation:{mission.asset_owner_id}",
                                json.dumps(corporation_info),
                            )
                            await rdm().redis.expire(
                                f"eveesi:corporations_corporation:{mission.asset_owner_id}",
                                60 * 60 * 24,
                            )
                        else:
                        ...
                        logger.info(
                            f"公司{mission.asset_owner_id}公开信息: {corporation_info}"
                        )
                        _append_mission(
                            {
                                "subject_type": mission.asset_owner_type,
                    ...
                    traceback.print_exc()
                    logger.error(
                        f"拉取同公司的任务失败: {e}, corp_id: {mission.asset_owner_id}"
                    )
                    _append_mission(
                        {
                            "subject_type": mission.asset_owner_type,
    ...
    async def get_all_asset_pull_mission_list(self) -> list[dict]:
        ...
        async for mission in await EveAssetPullMissionDBUtils.select_all():
            try:
                ...
                traceback.print_exc()
                logger.error(f"拉取所有任务失败: {e}, mission_id: {mission.id}")
                missions.append(
                    {
                        "subject_type": mission.asset_owner_type,
                        "subject_name": f"Unknown ({mission.asset_owner_id})",
    ...
    async def create_asset_pull_mission(
        self, user_name: str, asset_owner_type: str, asset_owner_id: int, active: bool
    ):
    ...
    async def get_station_info(self, station_id: int):
        ...
        # 获取缓存
        station_info_cache = await rdm().redis.hgetall(
            f"eveesi:universe_stations_station:{station_id}"
        )
        if not station_info_cache:
            ...
            await rdm().redis.hset(
                f"eveesi:universe_stations_station:{station_id}",
                mapping=station_info_cache,
            )
            await rdm().redis.expire(
                f"eveesi:universe_stations_station:{station_id}", 60 * 60 * 24
            )
        else:
    ...
    async def create_station_node(self, station_id: int):
        async with CREATE_STATION_SEMAPHORE:
            ...
            if not is_new:
                return
            system_info = await SdeUtils.get_system_info_by_id(
    ...
    async def _generate_all_nodes(
        self, assets_list: list[dict], mission_obj: M_EveAssetPullMission
    ):
        stucture_list = await NAU.get_structure_nodes()
        structure_item_id_list = [
            structure.get("item_id", None) for structure in stucture_list
        ]
        ...
        assets_to_create = [
            ...
            if asset["item_id"] not in structure_item_id_list
        ]
        ...
        type_name_tasks = [
            asyncio.create_task(get_type_name(asset)) for asset in assets_to_create
        ]
        ...
        for asset, type_name in zip(assets_to_create, type_names):
            asset.update(
                {"type_name": type_name, "owner_id": mission_obj.asset_owner_id}
            )
            nodes_data.append(
                {
                    "index": {
    ...
    async def _generate_all_locate_relation(
        self, assets_list: list[dict], mission_obj: M_EveAssetPullMission
    ):
        status_key = f"asset_pull_mission_status:{mission_obj.asset_owner_type}:{mission_obj.asset_owner_id}"
        structure_nodes = await NAU.get_structure_nodes()
        structure_item_id_list = [
            structure.get("structure_id", None) for structure in structure_nodes
        ]
        ...
        async with neo4j_manager().semaphore:
            if station_relations:
                await NIU.batch_link_nodes(
                    "Asset", "Station", "LOCATED_IN", station_relations
                )

            if solar_system_relations:
                await NIU.batch_link_nodes(
                    "Asset", "SolarSystem", "LOCATED_IN", solar_system_relations
                )

            if structure_relations:
                await NIU.batch_link_nodes(
                    "Asset", "Structure", "LOCATED_IN", structure_relations
                )

            if asset_relations:
                await NIU.batch_link_nodes(
                    "Asset", "Asset", "LOCATED_IN", asset_relations
                )

    async def _generate_forbidden_structure_node(
        self, mission_obj: M_EveAssetPullMission
    ):
        logger.info("开始生成无权限建筑节点")
        access_character = await CharacterManager().get_character_by_character_id(
            mission_obj.access_character_id
        )
        # 补全玩家建筑信息
        forbidden_structure_node_list = await NAU.get_forbidden_structure_node_list(
        ...
        await tqdm_manager.add_mission(
            "_generate_forbidden_structure_node", len(forbidden_structure_node_list)
        )
        for forbidden_structure_node in forbidden_structure_node_list:
            # 建筑信息
            logger.info(
                f"开始生成无权限建筑节点: {forbidden_structure_node['item_id']}"
            )
            structure_info_cache = await rdm().redis.hgetall(
                f"eveesi:universe_structures_structure:{forbidden_structure_node['item_id']}"
            )
            if not structure_info_cache:
                structure_info = await eveesi.universe_structures_structure(
                    access_character.ac_token, forbidden_structure_node["item_id"]
                )
                if structure_info:
                    logger.info(
                        f"建筑{forbidden_structure_node['item_id']}获取到建筑信息"
                    )
                    ...
                    logger.info(
                        f"建筑{forbidden_structure_node['item_id']}无权限，创建无权限建筑"
                    )
                    structure_info_cache = {
            ...
            if structure_info["solar_system_id"] != "unknown":
                system_info = await SdeUtils.get_system_info_by_id(
                    int(structure_info["solar_system_id"])
                )
                if not system_info:
                    raise KahunaException(
                        f"建筑{forbidden_structure_node['item_id']}无星系信息"
                    )
                solar_system_node = {
                    "system_id": system_info["system_id"],
                    "system_name": system_info["system_name"],
            ...
            structure_node = {
                ...
                "structure_type": await SdeUtils.get_name_by_id(
                    int(structure_info["type_id"])
                )
                if structure_info["type_id"] != "unknown"
                else "unknown",
            ...
            async with CREATE_STATION_SEMAPHORE:
                await NAU.merge_asset_to_structure_to_solar_system(
                    forbidden_structure_node, structure_node, solar_system_node
                )
    ...
    async def _update_structure_node(self, mission_obj: M_EveAssetPullMission):
        ...
        structure_asset_nodes = await NAU.get_structure_asset_nodes(
            mission_obj.asset_owner_id
        )
        status_key = f"asset_pull_mission_status:{mission_obj.asset_owner_type}:{mission_obj.asset_owner_id}"
        ...
        await tqdm_manager.add_mission(
            "_update_structure_node", len(structure_asset_nodes)
        )
        for node in structure_asset_nodes:
            structure_info_cache = await rdm().redis.hgetall(
                f"eveesi:universe_structures_structure:{node['item_id']}"
            )
            if not structure_info_cache:
                structure_info = await eveesi.universe_structures_structure(
                    access_character.ac_token, node["item_id"]
                )
                if structure_info:
            ...
            now_progress = await tqdm_manager.update_mission(
                "_update_structure_node", 1
            )
            await rdm().r.hset(
                status_key, "step_progress", now_progress / len(structure_asset_nodes)
            )
        await tqdm_manager.complete_mission("_update_structure_node")

    async def _update_forbidden_structure_node(
        self, mission_obj: M_EveAssetPullMission
    ):
        ...
        forbidden_structure_nodes = await NAU.get_forbidden_structure_nodes_by_owner(
            mission_obj.asset_owner_id
        )
        ...
        await tqdm_manager.add_mission(
            "_update_forbidden_structure_node", len(forbidden_structure_nodes)
        )
        for structure_node_data in forbidden_structure_nodes:
            ...
            # 尝试获取建筑信息
            structure_info_cache = await rdm().redis.hgetall(
                f"eveesi:universe_structures_structure:{structure_id}"
            )
            if not structure_info_cache:
                structure_info = await eveesi.universe_structures_structure(
                    access_character.ac_token, structure_id
                )
                if structure_info:
                    ...
                    await tqdm_manager.update_mission(
                        "_update_forbidden_structure_node", 1
                    )
                    continue
                await rdm().redis.hset(
                    f"eveesi:universe_structures_structure:{structure_id}",
                ...
                await rdm().redis.expire(
                    f"eveesi:universe_structures_structure:{structure_id}", 60 * 60 * 24
                )
            else:
                # 检查缓存中的信息是否有效（不是unknown）
                if (
                    structure_info_cache.get("solar_system_id") == "unknown"
                    or structure_info_cache.get("solar_system_id") == b"unknown"
                ):
                    ...
                    # 确保缓存中有system_id等信息
                    if (
                        "system_id" not in structure_info_cache
                        or structure_info_cache.get("system_id") == "unknown"
                    ):
                        ...
                        if solar_system_id and solar_system_id != "unknown":
                            ...
                            structure_info_cache["system_name"] = system_info[
                                "system_name"
                            ]
                            structure_info_cache["region_id"] = system_info["region_id"]
                            structure_info_cache["region_name"] = system_info[
                                "region_name"
                            ]
                            await rdm().redis.hset(
                                f"eveesi:universe_structures_structure:{structure_id}",
            ...
            system_id_value = structure_info.get("system_id") or structure_info.get(
                "solar_system_id"
            )
            ...
            # 更新节点
            async with CREATE_STATION_SEMAPHORE:
                success = await NAU.update_forbidden_structure_node(
                    structure_id, structure_node, solar_system_node
                )
                if success:
            ...
            await rdm().r.hset(
                status_key,
                "step_progress",
                now_progress / len(forbidden_structure_nodes),
            )
    ...
    async def processing_asset_pull_mission(self, mission_obj: M_EveAssetPullMission):
        ...
        assets = await pull_function(
            access_character.ac_token, mission_obj.asset_owner_id, status_key=status_key
        )
        assets_list = []
    ...
    async def _update_solar_system_connected_asset_nodes(
        self, mission_obj: M_EveAssetPullMission
    ):
        ...
        asset_nodes = await NAU.get_solar_system_connected_asset_nodes(
            mission_obj.asset_owner_id
        )
        ...
        await tqdm_manager.add_mission(
            "_update_solar_system_connected_asset_nodes", len(asset_nodes)
        )
        for node in asset_nodes:
            ...
            if not item_id:
                ...
                await tqdm_manager.update_mission(
                    "_update_solar_system_connected_asset_nodes", 1
                )
                continue
            ...
            # 尝试获取建筑信息
            structure_info_cache = await rdm().redis.hgetall(
                f"eveesi:universe_structures_structure:{item_id}"
            )
            if not structure_info_cache:
                structure_info = await eveesi.universe_structures_structure(
                    access_character.ac_token, item_id
                )
                if structure_info:
                    ...
                    await rdm().redis.expire(
                        f"eveesi:universe_structures_structure:{item_id}", 60 * 60 * 24
                    )
                else:
                ...
                # 检查缓存中的信息是否有效（不是unknown）
                if (
                    structure_info.get("solar_system_id") == "unknown"
                    or structure_info.get("system_id") == "unknown"
                ):
                    logger.info(f"Asset节点{item_id}缓存信息为unknown，尝试重新获取")
                    structure_info_new = await eveesi.universe_structures_structure(
                        access_character.ac_token, item_id
                    )
                    if structure_info_new:
                        logger.info(f"Asset节点{item_id}重新获取到建筑信息")
                        system_info = await SdeUtils.get_system_info_by_id(
                            structure_info_new["solar_system_id"]
                        )
                        structure_info_cache = {
                        ...
                        logger.info(
                            f"Asset节点{item_id}重新获取仍无法获取建筑信息，跳过"
                        )
                        await tqdm_manager.update_mission(
                            "_update_solar_system_connected_asset_nodes", 1
                        )
                    ...
                    # 确保缓存中有system_id等信息
                    if (
                        "system_id" not in structure_info
                        or structure_info.get("system_id") == "unknown"
                    ):
                        ...
                        if solar_system_id and solar_system_id != "unknown":
                            try:
                                ...
                                structure_info["system_name"] = system_info[
                                    "system_name"
                                ]
                                structure_info["region_id"] = system_info["region_id"]
                                structure_info["region_name"] = system_info[
                                    "region_name"
                                ]
                                await rdm().redis.hset(
                                    f"eveesi:universe_structures_structure:{item_id}",
                                    mapping=structure_info,
                                )
                            except (ValueError, TypeError) as e:
                                logger.warning(
                                    f"Asset节点{item_id}的solar_system_id无效: {solar_system_id}, 错误: {e}"
                                )
                                await tqdm_manager.update_mission(
                                    "_update_solar_system_connected_asset_nodes", 1
                                )
            ...
            await rdm().r.hset(
                status_key, "step_progress", now_progress / len(asset_nodes)
            )
        await tqdm_manager.complete_mission(
            "_update_solar_system_connected_asset_nodes"
        )
    ...
    async def get_asset_view_of_user(self, user_name: str):
        asset_view_list = []
        async for asset_view in await EveAssetViewDBUtils.select_by_user_name(
            user_name
        ):
            # 优先使用asset_container_id_list，如果没有则使用asset_container_id（向后兼容）
            container_list = (
                asset_view.asset_container_id_list
                if hasattr(asset_view, "asset_container_id_list")
                and asset_view.asset_container_id_list
                else []
            )
            if not container_list and asset_view.asset_container_id:
                ...
                container_list = [
                    {
                        "container_id": asset_view.asset_container_id,
                        "owner_id": asset_view.asset_owner_id,
                    }
                ]
            ...
            asset_view_list.append(
                {
                    ...
                    "tag": asset_view.tag,
                    "public": asset_view.public
                    if hasattr(asset_view, "public")
                    else False,
    ...
    async def _check_filter(self, asset: dict, asset_view: M_EveAssetView):
        ...
        for f in view_filter:
            if (
                f["type"] == "location_flag"
                and asset.get("location_flag", None) != f["value"]
            ):
                return False
            ...
            if f["type"] == "group":
                ...
                group_zh = await SdeUtils.get_groupname_by_id(
                    asset.get("type_id", None), True
                )
                if group != f["value"] and group_zh != f["value"]:
                    return False
            if f["type"] == "meta":
                meta = await SdeUtils.get_metaname_by_typeid(asset.get("type_id", None))
                meta_zh = await SdeUtils.get_metaname_by_typeid(
                    asset.get("type_id", None), True
                )
                if meta != f["value"] and meta_zh != f["value"]:
                    return False
            if f["type"] == "marketGroup":
                market_group_list = await SdeUtils.get_market_group_list(
                    asset.get("type_id", None)
                )
                market_group_list_zh = await SdeUtils.get_market_group_list(
                    asset.get("type_id", None), True
                )
                if (
                    f["value"] not in market_group_list
                    and f["value"] not in market_group_list_zh
                ):
                    return False
            if f["type"] == "category":
                category = await SdeUtils.get_category_by_id(asset.get("type_id", None))
                category_zh = await SdeUtils.get_category_by_id(
                    asset.get("type_id", None), True
                )
    ...
    async def _get_class_type(self, type_id: int):
        ...
        if "Salvage Materials" in market_group_list:
            ...
            "Standard Capital Ship Components" in market_group_list
            or "Advanced Capital Components" in market_group_list
        ):
            return "旗舰组件"
    ...
    async def get_asset_statistics_data(self, user_name: str, sid: str):
        ...
        for item in container_list:
            ...
            container_owner_pairs.append(
                [container_id, owner_id, pull_permission.tag, location_flag]
            )

        asset_dict = {
            container_id: {"container_id": container_id, "name": name, "assets": {}}
        ...
        for container_id, owner_id, _, location_flag in container_owner_pairs:
            ...
            asset_data = await NAU.get_asset_in_container_owner_list(
                [[container_id, owner_id, location_flag]]
            )
            for asset in asset_data:
                # 过滤掉蓝图拷贝(BPC)，只计算蓝图原图(BPO)和其他资产
                if asset.get("is_blueprint_copy", False):
                    continue
    ...
    async def save_asset_view_config(
        ...
        if container_list is not None:
            ...
            async for cp in await EveIndustryAssetContainerPermissionDBUtils.select_all_by_user_name(
                user_name
            ):
            ...
            for item in container_list:
                ...
                if key not in user_container_permissions:
                    invalid_containers.append(
                        f"container_id={container_id}, owner_id={owner_id}"
                    )
    ...
    async def create_asset_view_from_container_permission(
        self, user_name: str, container_tag: str
    ):
        ...
        async for (
            cp
        ) in await EveIndustryAssetContainerPermissionDBUtils.select_all_by_user_name(
            user_name
        ):
    ...
    async def create_asset_view_from_container_list(
        self, user_name: str, container_list: list, tag: str
    ):
        """从容器列表创建资产视图
