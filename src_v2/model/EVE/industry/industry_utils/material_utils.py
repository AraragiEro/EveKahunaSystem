...
async def get_material_type(type_id: int, sdm=None) -> str:
    """根据类型ID获取材料类型
    ...
    """
    group = await SdeUtils.get_groupname_by_id(type_id, pdm=sdm)
    category = await SdeUtils.get_category_by_id(type_id, pdm=sdm)
