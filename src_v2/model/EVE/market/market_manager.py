...
class MarketManager(metaclass=SingletonMeta):
                    ...
                    try:
                        ...
                        if isinstance(issued_raw, str):
                            ...
                            if issued_dt.tzinfo is not None:
                                issued_dt = issued_dt.astimezone(
                        ...
                        logger.error(
                            f"转换订单数据失败 region_id={region_id}, order_id={order.get('order_id')}: {e}", exc_info=True)
                        raise

            # 使用临时表方案保存数据
            await tqdm_manager.add_mission(f"save_orders_to_database_{region_id}", len(orders))

            async def progress_callback(processed_count: int):
                await tqdm_manager.update_mission(f"save_orders_to_database_{region_id}", processed_count)

            await EveMarketRegionOrdersDBUtils.save_orders_via_temp_table(
                region_id=region_id,
                orders_generator=orders_generator(),
    ...
    async def update_frt_price(self, main_character_id: int = None):
        ...
        # 扁平化订单列表，便于写库
        flat_frt_orders = [
            order for order_list in frt_order for order in order_list]

        # 生成 type_price_cache
        type_price_cache = {}
        for order in flat_frt_orders:
            type_id = order["type_id"]
            if type_id not in type_price_cache:
                type_price_cache[type_id] = {
                    "max_buy": 0,
                    "min_sell": 1000000000000000000000,
                }

            if order["is_buy_order"]:
                type_price_cache[type_id]["max_buy"] = max(
                    type_price_cache[type_id]["max_buy"], order["price"]
                )
            else:
                type_price_cache[type_id]["min_sell"] = min(
                    type_price_cache[type_id]["min_sell"], order["price"]
                )
    ...
    async def update_type_id_history_detale(self, region_id: int, type_id: int):
        ...
        try:
            ...
            # 计算日期范围
            # 过去2日到过去9日（7天数据）
            end_date_7d = (now - timedelta(days=2)).replace(hour=0,
                                                            minute=0, second=0, microsecond=0)
            start_date_7d = (now - timedelta(days=9)).replace(hour=0,
                                                              minute=0, second=0, microsecond=0)

            # 过去2日到过去32日（30天数据）
            end_date_30d = (now - timedelta(days=2)).replace(hour=0,
                                                             minute=0, second=0, microsecond=0)
            start_date_30d = (now - timedelta(days=32)
            ...
            avg_price_30d = weighted_sum_30d / \
                total_volume_30d if total_volume_30d > 0 else 0.0
    ...
    async def get_type_id_history_detail(self, region_id: int, type_id: int) -> dict:
        ...
        try:
            avg_price_7d = float(
                avg_price_7d_str) if avg_price_7d_str is not None else 0.0
        except (ValueError, TypeError):
            avg_price_7d = 0.0

        try:
            avg_price_30d = float(
                avg_price_30d_str) if avg_price_30d_str is not None else 0.0
        except (ValueError, TypeError):
            avg_price_30d = 0.0

        try:
            total_volume_7d = float(
                total_volume_7d_str) if total_volume_7d_str is not None else 0.0
        except (ValueError, TypeError):
            total_volume_7d = 0.0

        try:
            total_volume_30d = float(
                total_volume_30d_str) if total_volume_30d_str is not None else 0.0
        except (ValueError, TypeError):
    ...
    async def update_type_id_list_history_detale(self, region_id: int, type_id_list: list, force: bool = False):
        ...
        try:
            ...
            logger.info(
                f"市场历史统计计算完成 region_id={region_id}，已设置Redis标志，过期时间: {expire_seconds} 秒后（次日凌晨2点）")
