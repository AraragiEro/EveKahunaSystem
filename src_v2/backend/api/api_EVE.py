    ...
    try:
        ...
        if state:
            try:
                ...
                original_oauth_state = state_payload.get(
                    'oauth_state')  # 获取原始state
                logger.debug(
        ...
        # 记录接收到的回调信息用于调试
        logger.debug(
            f"收到EVE OAuth回调 - code: {code[:10]}..., user_id: {user_id}")
        ...
        # 完成后，将用户重定向回前端应用程序
        frontend_redirect_url = "https://" + CALLBACK_LOCAL_HOST + \
            "/setting/characterSetting/auth/close" if CALLBACK_LOCAL_HOST else None
...
async def get_auth_status():
    ...
    try:
        ...
        return {"status": 200, "authStatus": auth_status, "characterName": character_name}
    except KahunaException as e:
        traceback.print_exc()
        return jsonify({"status": 500, "message": str(e)}), 500
    except Exception as e:
